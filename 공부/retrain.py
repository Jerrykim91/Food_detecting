...
TensorFlow Hub의 이미지 모듈로 간단한 전송 학습.

이 예제는 이미지 분류기를 훈련시키는 방법을 보여줍니다.
이미지 특징 벡터를 계산하는 TensorFlow Hub 모듈. 기본적으로,
ImageNet에서 훈련 된 Inception V3에서 계산 한 특징 벡터를 사용합니다.
https://github.com/tensorflow/hub/blob/r0.1/docs/modules/image.md를 참조하십시오
더 많은 옵션.

최상위 레이어는 입력으로 2048 차원 벡터를받습니다 (가정으로 가정)
각 이미지에 대한 시작 V3). 이 위에 softmax 레이어를 훈련시킵니다
대표. softmax 레이어에 N 레이블이 포함 된 경우
바이어스 및 가중치에 대한 N + 2048 * N 모델 파라미터를 학습합니다.

다음은 클래스 이름이 포함 된 폴더가 있다고 가정하는 예입니다.
하위 폴더, 각 라벨마다 이미지가 가득합니다. flower_photos 폴더 예
다음과 같은 구조를 가져야합니다.
...
하위 폴더 이름은 적용 할 레이블을 정의하므로 중요합니다.
각각의 이미지이지만 파일 이름 자체는 중요하지 않습니다. (실제로
http://download.tensorflow.org/example_images/flower_photos.tgz 다운로드
tar xzf flower_photos.tgz를 실행하여 압축을 풉니 다.)

이미지가 준비되면 tensorflow-hub를 핍 설치하고
충분히 최신 버전의 tensorflow를 사용하여
이 같은 명령 :
...
image_dir 인수를 하위 폴더를 포함하는 폴더로 바꿀 수 있습니다.
이미지. 각 이미지의 레이블은 하위 폴더의 이름에서 가져옵니다.
에.

이것은 모든 TensorFlow에 의해로드되고 실행될 수있는 새로운 모델 파일을 생성합니다
프로그램 (예 : tensorflow / examples / label_image 샘플 코드).

기본적으로이 스크립트는 매우 정확하지만 비교적 크고
느린 시작 V3 모델 아키텍처. 이것으로 시작하는 것이 좋습니다
좋은 교육 데이터를 수집했는지 확인하지만 배포하려는 경우
자원이 제한된 플랫폼에서는`--tfhub_module` 플래그를
모바일 넷 모델. Mobilenet에 대한 자세한 내용은
https://research.googleblog.com/2017/06/mobilenets-open-source-models-for.html

예를 들면 다음과 같습니다.


부동 소수점 버전의 Mobilenet 실행 :
...
이 계측 모델은 다음을 통해 완전히 양자화 된 모바일 모델로 변환 할 수 있습니다
텐서 플로우 라이트.

다양한 파일로 선택할 수있는 다양한 모바일 넷 모델이 있습니다
크기 및 대기 시간 옵션.
  -첫 번째 숫자는 숫자를 제어하기 위해 '100', '075', '050'또는 '025'일 수 있습니다.
    뉴런 (숨겨진 층의 활성화); 무게의 수 (따라서
    파일 크기와 속도)는 그 제곱에 따라 줄어 듭니다.
    분수.
  -두 번째 숫자는 입력 이미지 크기입니다. '224', '192',
    크기가 작을수록 '160'또는 '128'은 빠른 속도를 제공합니다.

TensorBoard와 함께 사용하려면 :

기본적으로이 스크립트는 요약을 / tmp / retrain_logs 디렉토리에 기록합니다.

이 명령으로 요약을 시각화하십시오.

tensorboard --logdir / tmp / retrain_logs

Tensorflow Serving과 함께 사용하려면 --saved_model_dir 세트로이 도구를 실행하십시오.
모델 기본 경로 아래에서 점점 더 많은 번호의 수출 위치로
